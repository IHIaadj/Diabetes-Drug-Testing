...
def reduce_dimension_ndc(df, ndc_df):
    ...
    '''
    # df == original dataset 
    # ndc_df == final_ndc_lookup_table 
    dataframe = pd.merge(df, 
                         ndc_df[['Proprietary Name', 'NDC_Code']], 
                         left_on ='ndc_code', 
                         right_on = 'NDC_Code')
    
    dataframe['generic_drug_name'] = dataframe['Proprietary Name']
    dataframe = dataframe.drop(['NDC_Code', 'Proprietary Name'], axis=1) # drop duplicates 
    
    return dataframe


#Question 4
def select_first_encounter(df, patient_key='patient_nbr'):
    '''
